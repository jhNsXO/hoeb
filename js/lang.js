...
(() => {
    ...
    const detectLanguage = () => {
        try {
            const match = window.location.search.match(/[?&]lang=(de|en)(?:&|$)/);
    ...
    const updatePageLinks = (lang) => {
        document.querySelectorAll('a[href]').forEach((link) => {
            ...
            const cleanPath = path.replace(/[?&]lang=[^&]*/g, '').replace(/\?$/, '');
            const sep = cleanPath.includes('?') ? '&' : '?';
            const hashPart = hash ? `#${hash}` : '';
            link.setAttribute('href', `${cleanPath}${sep}lang=${lang}${hashPart}`);
        });
    };
    ...
    const applyLanguage = (lang) => {
        ...
        try {
            if (window.history && window.history.replaceState) {
                const cleanSearch = window.location.search.replace(/[?&]lang=[^&]*/g, '').replace(/\?$/, '');
                const sep = cleanSearch ? (cleanSearch.includes('?') ? '&' : '?') : '?';
                const newSearch = cleanSearch ? `${cleanSearch}${sep}lang=${lang}` : `?lang=${lang}`;
    ...
    const initSwitcher = () => {
        ...
        switcher.innerHTML = `
            <button type="button" class="lang-btn" data-switch="en" title="English" aria-label="Switch to English">
                <img src="images/gb.svg" alt="English" class="flag-icon" width="20" height="14">
                <span>EN</span>
            </button>
        ...
        switcher.addEventListener('click', (e) => {
            ...
            if (!btn) return;
            e.preventDefault();
            const targetLang = btn.getAttribute('data-switch');
